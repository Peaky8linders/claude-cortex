...
export class KnowledgeGraph {
    ...
    addEdge(source, target, type, context = "") {
        // Ensure nodes exist
        ...
        const edge = { source, target, type, weight: 0.5, context, timestamp: Date.now() };
        this.edges.push(edge);
        return edge;
    }
    updateTokenCost(nodeId, tokens) {
        const node = this.nodes.get(nodeId);
        if (node)
            node.tokenCost += tokens;
    }
    setQualityImpact(nodeId, impact) {
        const node = this.nodes.get(nodeId);
        if (node)
            node.qualityImpact = Math.max(-100, Math.min(100, impact));
    }
    // ── Queries ──
    getNode(id) { return this.nodes.get(id); }
    getAllNodes() { return [...this.nodes.values()]; }
    getAllEdges() { return [...this.edges]; }
    getNodesByType(type) { return this.getAllNodes().filter(n => n.type === type); }
    getNeighbors(nodeId) {
    ...
    detectClusters() {
        ...
        for (const [nodeId] of this.nodes) {
            ...
            if (cluster.size > 0) {
                ...
                const dominantType = Object.entries(typeCounts).sort((a, b) => b[1] - a[1])[0]?.[0] ?? "file";
    ...
    computeClusterCohesion(nodeIds) {
    ...
    computeQualityScore(nodes, edges) {
        if (nodes.length === 0)
            return 50;
        ...
        if (budget.efficiency < 0.5)
            score -= 15;
        else if (budget.efficiency < 0.7)
            score -= 8;
    ...
    generateRecommendations(nodes, edges, budget, clusters) {
        const recs = [];
        let recId = 0;
        // R1: Files read but never modified → may be stale context
        const readOnlyFiles = nodes.filter(n => n.type === "file" &&
        ...
        const unlinkedDecisions = nodes.filter(n => n.type === "decision" && (this.adjacency.get(n.id)?.size ?? 0) === 0);
        ...
        const largeFileReads = nodes.filter(n => n.type === "file" && n.tokenCost > 4000 &&
        ...
        const simpleOpsOnHeavyModel = toolNodes.filter(n => n.name.includes("Read") || n.name.includes("Search") || n.name.includes("Glob"));
