...
export class ContextHubIntegration {
     ...
     */
    processBashCommand(command, output) {
     ...
     */
    detectHallucinations(codeContent) {
        const recs = [];
    ...
    recordAnnotation(docId, annotation) {
        const existing = this.docUsage.get(docId);
        if (existing) {
