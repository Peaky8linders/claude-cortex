...
export class HookProcessor {
    ...
    process(event) {
        ...
        const hookNode = this.graph.addNode(`hook:${event.hook_event_name}`, event.hook_event_name, "hook", { session_id: event.session_id });
...
function estimateToolTokens(toolName, input) {
    if (!input)
        return 50;
    const str = JSON.stringify(input);
    return Math.floor(str.length / 4);
}
function estimateFileTokens(input) {
    if (!input)
        return 0;
...
function extractDecisionsFromCode(graph, content) {
