 ...
 */
import { estimateTokensForEntry, groupByPromptTurn } from "./session-reader.js";
 ...
 */
export function estimateCost(tokensEst, model) {
 ...
 */
export function computeSessionCost(entries) {
    const byModel = {};
    let totalTokens = 0;
    let totalCost = 0;
 ...
 */
export function estimateCacheAwareCost(entries, sessionType) {
    ...
    for (let i = 0; i < turns.length; i++) {
        ...
        const turnNoCacheCost = (inputTokens / 1_000_000) * pricing.input_per_m +
 ...
 */
export function detectCacheAnomaly(entries, sessionType) {
    const turns = groupByPromptTurn(entries);
    if (turns.length < 2) {
        return {
            detected: false,
    ...
    const subsequentTokens = turns.slice(1).map(turn => turn.reduce((s, e) => s + estimateTokensForEntry(e), 0));
