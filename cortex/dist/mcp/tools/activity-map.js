 ...
 */
import { getSessionEntries, readSkills, } from "../data/session-reader.js";
/**
 * Estimate activation durations using index-based next-event lookup.
 * Entries must be pre-sorted by timeMs.
 */
function estimateActivations(timedEntries, filterFn) {
    const activations = [];
    for (let i = 0; i < timedEntries.length; i++) {
        if (!filterFn(timedEntries[i].entry))
            continue;
...
export function computeActivityMap(sessionId, includeHooks = true, includeSkills = true, knowledgeDir) {
    ...
    if (includeHooks) {
        ...
        for (const entry of entries) {
            ...
            if (!hookGroups.has(hookName))
                hookGroups.set(hookName, []);
...
function computeConcurrencyPeak(timedEntries) {
    if (timedEntries.length < 2)
        return timedEntries.length;
    let maxConcurrent = 1;
    let j = 0;
    for (let i = 0; i < timedEntries.length; i++) {
