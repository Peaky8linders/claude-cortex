...
export function computeTokenTimeline(
  ...
  const bucketMap = new Map<number, { tokens: number; tool_counts: Record<string, number>; entries: JournalEntry[] }>();

  for (const entry of entries) {
    const entryTime = new Date(entry.ts).getTime();
    const minuteBucket = Math.floor((entryTime - startTime) / 60_000);

    if (minuteBucket < 0 || minuteBucket > windowMinutes) continue;
  ...
  for (let m = 0; m <= maxMinute; m++) {
    ...
    if (bucket?.tool_counts) {
      ...
      if (sorted.length > 0) dominantTool = sorted[0][0];
