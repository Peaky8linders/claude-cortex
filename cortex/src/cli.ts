...
import { HookProcessor, type HookEvent } from "./hooks/processor.js";
...
function main() {
  ...
  switch (cmd) {
    case "status": cmdStatus(isJson); break;
    case "graph": cmdGraph(); break;
    case "recommend": cmdRecommend(isJson); break;
    case "snapshot": cmdSnapshot(); break;
    case "install-hooks": cmdInstallHooks(); break;
    case "ingest": cmdIngest(args[1], isJson); break;
    ...
    for (const line of content.split("\n").filter(Boolean)) {
      try {
        ...
        if (event.hook_event_name || event.type) {
          processor.process({
            ...event,
            hook_event_name: event.hook_event_name ?? (event as Record<string,unknown>).type as string ?? "unknown",
...
function cmdStatus(isJson: boolean) {
  ...
  if (metrics.recommendations.length > 0) {
    ...
    const icons: Record<string, string> = { critical: "🔴", warning: "🟠", optimize: "🟡", suggestion: "🔵" };
...
function cmdRecommend(isJson: boolean) {
  const graph = buildGraphFromSession();
  const metrics = graph.computeMetrics();
...
function cmdInstallHooks() {
  ...
  for (const event of events) {
    ...
    const existing = (hooks[event] as Array<Record<string, unknown>>).some(
      h => JSON.stringify(h).includes("cortex/events")
    );
    if (!existing) {
      (hooks[event] as unknown[]).push(cortexHook(event));
    }
  }
  ...
  if (!existsSync(".cortex/events")) mkdirSync(".cortex/events", { recursive: true });
...
function cmdIngest(file: string | undefined, isJson: boolean) {
...
function inferEventType(data: Record<string, unknown>): string {
  if (data.tool) return "PostToolUse";
  if (data.message && typeof data.message === "string") return "UserPromptSubmit";
  if (data.role === "user") return "UserPromptSubmit";
  if (data.role === "assistant") return "PostToolUse";
  return "unknown";
}
