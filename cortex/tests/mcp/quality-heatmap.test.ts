...
describe("computeQualityHeatmap", () => {
  it("returns synthetic result when no context and no snapshot", () => {
  ...
  it("returns all 7 dimensions", () => {
    const result = computeQualityHeatmap("test context for quality analysis");
  ...
  it("includes economics data", () => {
    const result = computeQualityHeatmap("sample context text for analysis");
  ...
  it("classifies health status correctly", () => {
    const result = computeQualityHeatmap("test");
    expect(["healthy", "degraded", "critical"]).toContain(result.health_status);
  });

  it("radar_labels and radar_values have matching lengths", () => {
    const result = computeQualityHeatmap("some context");
