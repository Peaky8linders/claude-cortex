...
DIFF_STAT=$(git diff --stat "$DIFF_REF" 2>/dev/null || echo "")
FILES_CHANGED=$(git diff --name-only "$DIFF_REF" 2>/dev/null | wc -l)
LINES_ADDED=$(git diff "$DIFF_REF" --numstat 2>/dev/null | awk '{sum+=$1} END {print sum+0}')
...
if echo "$CHANGED_FILES" | grep -q "^brainiac/"; then
  if python3 -m pytest "$REPO_ROOT/brainiac/tests/" -x -q --tb=no 2>/dev/null; then
    : # tests passed
  else
    TEST_PENALTY=20
  fi
...
if echo "$CHANGED_FILES" | grep -q "^contextscore/"; then
  if python3 -m pytest "$REPO_ROOT/contextscore/tests/" -x -q --tb=no 2>/dev/null; then
    : # tests passed
  else
    TEST_PENALTY=20
  fi
